...
router.post('/postDat/:xml', async(req, res, next) => {
  const inputXml = parser.toXml(req.params.xml)
  ...
  var xml1 =
     ...
     '<ns2:messageId>urn:x-mrn:stm:portcdm:message:0509ab7c-22fb-43a1-a21b-dc36bfd45d32</ns2:messageId>' +
     ...
     '</ns2:locationState>' +
     '</ns2:portCallMessage>'
  const response = await api.post('http://192.168.56.101:8080/dmp/mss/state_update', xml1, {
    headers: {
      'X-PortCDM-UserId': 'porter',
      'X-PortCDM-Password': 'porter',
      'X-PortCDM-APIKey': 'eeee',
